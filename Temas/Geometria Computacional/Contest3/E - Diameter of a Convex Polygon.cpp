...
int main(){
	...
	auto check=[&](int i,int j)->bool{
        line base(pol[i],pol[j]);
        line nxt(pol[i],pol[(i+1)%n]),bef(pol[j],pol[(j-1+n)%n]);
        if(nxt.linecrossline(bef)==2){
            point cs=nxt.crosspoint(bef);
            if(base.relation(cs)!=2) return false;
        }
        nxt=line(pol[j],pol[(j+1)%n]);
        bef=line(pol[i],pol[(i-1+n)%n]);
        if(nxt.linecrossline(bef)==2){
            point cs=nxt.crosspoint(bef);
            if(base.relation(cs)!=1) return false;
        }
        return true;
	};
	long double diam=0;
	int i,j=1;
	while(j<n && !check(0,j)) j++;
	for(int i=0;i<n;i++){
        diam=max(diam,pol[i].distance(pol[j]));
        while(check(i,(j+1)%n)){
            j=(j+1)%n;
            diam=max(diam,pol[i].distance(pol[j]));
        }
