...
int main(){
	...
	while(t--){
        ...
        L s1(P(x[0],y[0]),P(x[1],y[1])),s2(P(x[2],y[2]),P(x[3],y[3]));
        if(intersectSS(s1,s2)){
            P cs=crosspoint(s1,s2);
            bool ok=true;
            for(int i=0;i<4 && ok;i++){
                L line(P(0,y[i]),P(1,y[i]));
                if(y[i]>imag(cs) && intersectLS(line,s1) && intersectLS(line,s2)){
                    ok=false;
                    P cs1=crosspoint(line,s1);
                    P cs2=crosspoint(line,s2);
                    cout<<abs(real(cs1)-real(cs2))*(y[i]-imag(cs))/2<<"\n";;
                    break;
                }
