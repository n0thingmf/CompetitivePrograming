...
int main(){
	...
	vector<L> outwall;
	outwall.pb(L(P(0,0),P(100,0)));
	outwall.pb(L(P(100,0),P(100,100)));
	outwall.pb(L(P(0,0),P(0,100)));
	outwall.pb(L(P(0,100),P(100,100)));
	vector<P> door;
	vector<int> veto;
	for(int i=0;i<4;i++){
	    vector<P> pts;
        ...
        pts.pb(outwall[i][1]);
        for(int j=0;j<n;j++){
            if(intersectSS(outwall[i],wall[j])){
                pts.pb(crosspoint(outwall[i],wall[j]));
            }
        }
        sort(all(pts));
        for(int j=0;j<pts.size()-1;j++){
            veto.pb(-1);
            door.pb((pts[j]+pts[j+1])*0.5);
        }
	}
	int out=door.size();
	//cout<<out<<"\n";
    for(int i=0;i<n;i++){
        ...
        for(int j=0;j<n;j++){
            if(i==j) continue;
            if(intersectSS(wall[i],wall[j])){
                pts.pb(crosspoint(wall[i],wall[j]));
            }
        }
        sort(all(pts));
    ...
    door.pb(P(x,y));
    veto.pb(-1);
    vector<vector<int> > adj(door.size());
    for(int i=0;i<door.size();i++){
        //cout<<"punto numero "<<i<<" ---------------------\n";
        for(int j=max(out,i+1);j<door.size();j++){
            bool ok=true;
            for(int k=0;k<n;k++){
                if(k==veto[i] || k==veto[j]) continue;
                if(intersectSS(L(door[i],door[j]),wall[k])){
                    //cout<<door[i]<<"-"<<door[j]<<" intersect with "<<wall[k][0]<<"-"<<wall[k][1]<<"\n";
                    ok=false;
                    break;
                }
            }
    ...
    cout<<"Number of doors = "<<dist[door.size()-1]<<"\n";
	return 0;
