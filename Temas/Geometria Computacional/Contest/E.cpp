...
int main(){
	...
	while(true){
        ...
        for(int i=0;i<n;i++){
            ...
            cin>>x>>y0>>y1>>y2>>y3;
            lines.pb(L(P(x,0),P(x,y0)));
            lines.pb(L(P(x,y1),P(x,y2)));
            lines.pb(L(P(x,y3),P(x,10)));
            points.pb(P(x,y0));
            points.pb(P(x,y1));
            points.pb(P(x,y2));
            points.pb(P(x,y3));
        ...
        for(int i=0;i<m;i++){
            for(int j=i+1;j<m;j++){
                if(real(points[i])==real(points[j])) continue;
                L x(points[i],points[j]);
                bool ok=true;
                for(int xd=0;xd<lines.size();xd++){
                    L y=lines[xd];
                    if(x[0]==y[0] || x[0]==y[1] || x[1]==y[0] || x[1]==y[1]) continue;
                    if(intersectSS(x,y)) ok=false;
                }
                if(ok){
                    double w=abs(points[j]-points[i]);
                    adj[i].pb(make_pair(j,w));
                }
            }
        }
        ...
        while(!q.empty()){
            double now_d=q.top().first;
            int now=q.top().second;
            q.pop();
            if(now_d!=dist[now]) continue;
            for(int xd=0;xd<adj[now].size();xd++){
                double w=adj[now][xd].second;
                int next=adj[now][xd].first;
