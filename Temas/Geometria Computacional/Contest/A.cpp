...
int main(){
    ...
    int n,m,x1,y1,x2,y2,t=0;
    while(cin>>n && n>0){
        if(t) cout<<"\n";
        t++;
        cin>>m>>x1>>y1>>x2>>y2;
        vector<pair<P,P> > line(n+1);
        for(int i=0;i<n;i++){
            int u1,u2;
            cin>>u1>>u2;
            line[i].first=P(u2,y2);
            line[i].second=P(u1,y1);
        }
        line[n].first=P(x2,y2);
        line[n].second=P(x2,y1);
        ...
        for(int i=0;i<m;i++){
            ...
            P toy(x,y);
            for(int j=0;j<=n;j++){
                if(ccw(line[j].first,line[j].second,toy)==1){
                    cnt[j]++;
                    break;
                }
            }
