...
struct point
{
    ...
    point rotate(point p,double angle)//Rotar un ángulo angle en sentido contrario a las agujas del reloj alrededor del punto p
    {
        point v=this->sub(p);
...
struct line
{
    ...
    double angle()//inclinación en línea recta 0<=angle<180
    {
...
struct polygon
{
    ...
    int circlecover(double r)//cobertura del círculo unitario
    {
        int ans=0,i,j;
...
{
    circle c[maxn];
    double ans[maxn];//ans[i]Indica el área cubierta i veces
    double pre[maxn];
    int n;
    circles(){}
    void add(circle cc)
    ...
    void init_or()//el área del círculo y quitar el círculo cerrado
    {
        int i,j,k=0;
        bool mark[maxn]={0};
        for (i=0;i<n;i++)
    ...
    void init_and()//El área del círculo se corta para eliminar el círculo interior
    {
        int i,j,k=0;
        bool mark[maxn]={0};
        for (i=0;i<n;i++)
...
struct line3
{
    ...
    point3 rotate(point3 p,double ang)//p alrededor de este vector ángulo arg en sentido contrario a las agujas del reloj
    {
...
struct plane
{
    ...
    bool pointonplane(point3 p)//si el punto está en el plano
    {
    ...
    //1 está en el borde
    //2 adentro
    int pointontriangle(point3 p)//Si el punto está en el triángulo espacial abc
    {
    ...
    //relación entre dos planos
    //0 intersección
    //1 paralelo pero no coincidente
    //2 superposición
    ...
    double angleplane(plane f)//ángulo entre dos planos
    {
        return acos(o.dot(f.o)/(o.len()*f.o.len()));
    }
    double dispoint(point3 p)//distancia punto a plano
    {
        return fabs(p.sub(a).dot(o)/o.len());
    }
    point3 pttoplane(point3 p)//retorna el punto más cercano al plano(proyeccion)
    {
        line3 u=line3(p,p.add(o));
        crossline(u,p);
        return p;
    }
    int crossline(line3 u,point3 &p)//Intersección de plano y recta
    {
    ...
    int crossplane(plane f,line3 &u)//Intersección de plano y plano
    {
        point3 oo=o.det(f.o);
        point3 v=o.det(oo);
